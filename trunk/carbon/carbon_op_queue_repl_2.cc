...
carbon_op_queue_repl_2::handleRequest(requestTypes::NetRequest * data, unsigned long dataSize)
{
  ...
  {
    cout<< "Repl Queue 2 :: MAC authentication with primary failed; Discarding the request" << std::endl;
    return;
  } 
...
carbon_op_queue_repl_2::my_init()
{
   ...
   string repl_2_shared_file("repl_2_shared_keys.txt");
   repl_2_shared_file = string(KEYS_DIR) + repl_2_shared_file;
   replicaKeyHash = utilities::ReplicaKeyHash_p(new utilities::ReplicaKeyHash(repl_2_shared_file));
}
